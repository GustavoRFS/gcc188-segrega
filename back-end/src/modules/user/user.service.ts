...
export class UsersService {
  ...
  public static async createUser(user: UserInput): Promise<UserOutput> {
    const registerToken = crypto.randomBytes(20).toString("hex");
    const {
      raw: [{ id }],
    } = await UsersRepository.createUser(
      Object.assign(user, { registerToken })
    );
  ...
  public static async login(user: UserLogin): Promise<UserLoginOutput> {
  ...
  public static async confirmUser(
    ...
    const u = await UsersRepository.updateUser(
      response.id,
      Object.assign(response, { password })
    );
